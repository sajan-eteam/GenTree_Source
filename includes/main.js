...
(function($) {
   ...
   vscroll_contenuto = function() {
            var altezza = $(window).height()-$("#header").outerHeight();
            $("#container, #left_column").height(altezza);
            $("#tabella").height(altezza-$("#tabella_head").outerHeight());
...
/*new one*/
	selection_color = function(element){
        var row
        if($("#tabella").hasClass("table_thumb_view")||$("#tabella").hasClass("table_list_view")){
