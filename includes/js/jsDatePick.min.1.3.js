...
JsDatePick.prototype.setConfiguration=function(a){
    ...
    this.oConfiguration.imgPath=(g_jsDatePickImagePath.length!=null)?g_jsDatePickImagePath:"includes/img/";
...
JsDatePick.prototype.getDOMCalendarStripped=function(){
	...
	if(document.getElementById('date_p')) {
		this.setC(q,"divreset");
		q.innerHTML = '<br /> <input type="button" onclick="return resetdate();" class="reset_btn" value="clear">';		
	}
    this.tooltip=f;
    e.appendChild(i);
...
g.onmouseout=function(){
    ...
    if(parseInt(this.getAttribute("isToday"))==1){
        ...
        this.style.background="url(includes/"+d.oConfiguration.imgPath+i+"_dayNormal.gif) left top no-repeat"
        }else{
