...
$(document).ready(function() {
	...
	$("#taggable").mousedown(function (e){
		...
		var boxh=$("#box"+i).height();
         ...
         var ajaxOpts = {  
         	type: 'POST',
           url: 'insta_album_detail.php',
		   data: 'tag=1'+'&x_cord='+x1+'&y_cord='+y1,
           success: function(data)
           {
           		
			    var box   =  $('<div id="box'+ i +'"class="tagbox">' + i +'</div>').hide();
                var input =  $('<div id="input'+ i +'"class="tag_list"><p class="number" id="num'+ i +'">' + i + '</p><input type="text" id="tagged_name" value=""/><div class="remove" onclick="removebox('+i+')">x</div></div>');
