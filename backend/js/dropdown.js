...
var boxdetail = [];
	select_item = function(myChoice){
		...
		var returnVal	=	tagListCheck(personv);
		if(returnVal == 0) {
	...
	function tagListCheck(elem){
		...
		$("div[id^='box']").each(function() {
						...
						var userName 		= $("#input"+myvar[1]).find("#username").text();
						if(value == elem) 
						{
							alert(userName+ " E’ già presente nella lista dei tag");
							existsFlag	=	1;
						}						
					});
		return existsFlag;
