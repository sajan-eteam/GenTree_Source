...
function getAllUsers($objuser,$url,$condition,$reqfor)
{
	...
	for($i=0;$i<count($alluser);$i++)
	{
		...
		$thresholdStatus	    =  	(isset($alluser[$i]['threshold_status']) && $alluser[$i]['threshold_status'] !='') ? $alluser[$i]['threshold_status'] : '1';
...
function getAllPairs($objuser, $condition, $reqFor)
{
	...
	for($i=0; $i<count($objpairs); $i++)
	{
		...
		$TresholdStatus 		= $objpairs[$i]['threshold_status'];
		...
		if($userPublishStatus == 1) {
			...
			if($count>0)
			{
				...
				$arrays =  array('m'=>$father_id, 'f'=>$mother_id, 'Relation_type'=>$reltype, 'Marriage_year'=>$marriageYear, 'TSN' => $TresholdStatus, 'offsprings'=>$offspringarray);			
...
function getCheckSum($photoObject){

	$hexString = $photoObject;#sample value
...
function getInstaImages($objInstaAlbum, $objInstaTags, $objLike, $objappuser, $uploadPath, $condition, $reqfor, $loginUserId){
	...
	for($i=0;$i<count($image_list);$i++){
            ...
            $image_title		=	getCleanString($image_title);
            $image_desc			=	getCleanString($image_desc);
            $user_fullname		=   $objappuser->appuserdetail($user_id);
            $fullname           =   $user_fullname['sys_name']." ".$user_fullname['sys_surname'];
			...
			if($reqfor=='sync' || $reqfor=='fullsync')
			{
				...
				else if($trash == 0 && $imagePublish == 1){
			...
			$like_list 		=   $objLike->instaLikeImages($image_id, $condition);

            $imagearray[$i]     =   array(	'imageId' 			=>	$image_id,
            								'fullname'			=>  getCleanString($fullname),
											'image_name' 		=>	$image_name,
											'image_date' 		=>	$image_date,
											'image_title' 		=>	$image_title,
											'image_description' =>	$image_desc,
            ...
            $tagDetails			=	array();
			$imageTagDetails 	=	$objInstaTags->taggingDetails($image_id, $condition);
			for($tag_count=0; $tag_count<count($imageTagDetails);$tag_count++)
			{
				$tag_name			=	$imageTagDetails[$tag_count]['tagged_name'];
				$tag_id				=	$imageTagDetails[$tag_count]['id_tag'];
				$image_id 			=   $imageTagDetails[$tag_count]['id_image'];
                $tag_x				=	$imageTagDetails[$tag_count]['tag_x_cord'];
				$tag_y              =   $imageTagDetails[$tag_count]['tag_y_cord'];
				$tag_width          =   $imageTagDetails[$tag_count]['tag_width'];
				$tag_height         =   $imageTagDetails[$tag_count]['tag_heigth'];
				$image_width		=	$imageTagDetails[$tag_count]['image_width'];
				$image_height		=	$imageTagDetails[$tag_count]['image_height'];
				$image_x			=	$imageTagDetails[$tag_count]['image_x'];
				$image_y            =   $imageTagDetails[$tag_count]['image_y'];
				$tag_counter 		=	$imageTagDetails[$tag_count]['tag_counter'];
				$device_name 		=	$imageTagDetails[$tag_count]['device_name'];
				$last_modified		=	$imageTagDetails[$tag_count]['LastModifiedTime'];
				$del_sttaus         =   $imageTagDetails[$tag_count]['Trash'];
				if($del_sttaus == 0){$tag_status ='delete';}else{$tag_status = 'tagsuccess';}
				$tagDetails[$tag_count]['tagName']			=	$tag_name;
				$tagDetails[$tag_count]['tagId'] 			= 	$tag_id;
				$tagDetails[$tag_count]['imageId'] 			= 	$image_id;
                $tagDetails[$tag_count]['tag_x_cord'] 	    = 	$tag_x;
                $tagDetails[$tag_count]['tag_y_cord'] 	    = 	$tag_y;
                $tagDetails[$tag_count]['tag_width'] 	    = 	$tag_width;
                $tagDetails[$tag_count]['tag_height'] 	    = 	$tag_height;
                $tagDetails[$tag_count]['image_width']		=	$image_width;
				$tagDetails[$tag_count]['image_height']		=   $image_height;
				$tagDetails[$tag_count]['image_x']			=	$image_x;
				$tagDetails[$tag_count]['image_y']			=	$image_y;
				$tagDetails[$tag_count]['tag_counter']		=	$tag_counter;
				$tagDetails[$tag_count]['tag_status']		=	$tag_status;
				$tagDetails[$tag_count]['device_name']		=	$device_name;
				$tagDetails[$tag_count]['LastModifiedTime']	=	$last_modified;
                $tagDetails[$tag_count]['delStatus']        =   $del_sttaus;
				
			}
		$imagearray[$i]['tagDetails'] = $tagDetails;
		for($j=0;$j<count($like_list);$j++){
            		$userrId	= $like_list[$j]['user_id'];
            		$likeStatus = $like_list[$j]['like_status'];	

            		$likearray[$j]['d']  			=  $userrId;
            		$likearray[$j]['likeStatus'] 	=  $likeStatus;	
            	}
            		$imagearray[$i]['likeInfo']  = $likearray; 
        }
	 return $imagearray;
}

function getInstaAllusers($objLike, $objappuser, $condition, $reqfor){

		$allusers = array();
        $getAllusr = $objLike->getInstaAllusers($condition);
        for($i=0;$i<count($getAllusr);$i++){

			$user_id 			=	$getAllusr[$i]['user_id'];
			$likeStatus         =   $getAllusr[$i]['like_status'];
			$user_fullname		=   $objappuser->appuserdetail($user_id);
            $fullname           =   $user_fullname['sys_name']." ".$user_fullname['sys_surname'];

            $allusers[$i]		=   array( 'd' 			=> $user_id,
            							   'n' 			=> getCleanString($fullname),
...
function getInstaImageAllusers($objInstaAlbum, $objappuser, $condition, $reqfor){

		$allusers = array();
        $getAllusr = $objInstaAlbum->getInstaImgAllusers($condition);
        for($i=0;$i<count($getAllusr);$i++){
